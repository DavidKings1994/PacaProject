                    ...
                    <li class="dropdown" v-if="logged && rol == 'user'">
                        ...
                        <ul class="dropdown-menu" id="notificationList">
                            ...
                            <li v-for="notification in notifications" v-if="showNotification(notification)" :data-id="notification.id" v-bind:class="{ active: notification.status == 1 }">
                                <router-link :to="'/user/' + userId + '/requests#' + notification.id" :id="'nav-requests' + notification.id">
                                    <div class="media">
...
export default {
    ...
    created: function() {
        var pusher = new Pusher('445a66cb7f6ee1fa6f0c', {
            cluster: 'us2'
        });

        var channel = pusher.subscribe('pacapillarsNotifications');
